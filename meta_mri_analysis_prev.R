...
#queries:
# backtransform I2?
# make sure inverse method used to weight studies and not sample size [can check when requesting 'summary'. appear to be iinverse by default ]
# fix meta regression: psychosis duration not fully functional
# can select studies for inclusion using subset function e.g. "(pes_fep_ab_summary, subset = 1:4)
...
# general format: individual effect size =escalc(xi=cases, ni=total, data=dat,
# measure="PR"/"PLO"/"PFT")
...
ies_da_fep_ab=escalc(xi=fep_abnormal, ni=fep_total, data=data, measure="PFT", add=0) #all abnormalities
ies_da_fep_cr_ab=escalc(xi=fep_cr_abnormal, ni=fep_total, data=data_cr, measure="PFT", add=0) # clinically relevant abnormalities

#individual effect sizes and their sampling variances
#NB When a study contains proportions equal to 0, R will automatically add 0.5 to the observed data (i.e., the number of
#event of interest, namely the cases vari-able). Since the double-arcsine
#transformation does not require any adjustments to be made to the data in such
#a situation, we can explicitly switch add=0.5 to add=0 to stop the default
#adjustment. 


#Pooled effect size using transformed double-arcsine transformed summary effect size. Random effects model using Dl estimator
pes_da_fep_ab=rma(yi, vi, data=ies_da_fep_ab, method="DL", level=95) # all abnormalities
...
###################  meta analysis of proportions - quick version [?redundant, ? remove] ################### 
...
# #metareg: psychosis duration all abnormalities 
...
# does not work:
#   eggers_reg_fep_ab<-regtest(pes_fep_cr_ab_summary) #  requires  'vi' or 'sei' argument.
