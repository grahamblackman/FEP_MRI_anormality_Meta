...
################### meta analysis of proportions using meta-prop

# This is redundant given the same results reported above but included for completeness and
# cross-checking
...
# query: to transform the model coefficients and interval bound eg exp?0 (default: no)

####  any abnormality
# L1O based on 'metafor::leave1out' function 
leave1out_fep_ab <- leave1out(pes_da_fep_ab, digits = 2) # works nb based on DA transformation)
print(leave1out_fep_ab)
# forest(x =leave1out_fep_ab$estimate, ci.lb = leave1out_fep_ab$ci.lb, ci.ub = leave1out_fep_ab$ci.ub) 
...
# plot(infuence_fep_ab) # plot [influential case flagged in red based on cut offs described by ]
...
# plot(infuence_fep_cr_ab) # plot [influential case flagged in red based on cut offs described by ]
...
# NB  [NB if variable containns NA may prompt error]
...
#metareg: mean age any abnormality
  metareg_age_fep_ab <- rma(yi, vi, data = ies_da_fep_ab, mods = ~age_fep, method="DL")
    #(Added by MTD) Warning message: Studies with NAs omitted from model fitting
...
# ?QUERY I've commented the following out as it fails and hence prevents the rest of the script
# from being executed in the source command in the main script.
#
# Error is: "Number of parameters to be estimated is larger than the number of observations"

# #metareg: psychosis duration clinically relevant abnormalities 
...
########## sensitivity analyses (robustness) ########## 
# No studies with poor quality according to quality assessment by MTD
...
# SA not performed:
#studies that were based on a research sample: see subgroup analysis
...
# trim and fill based for CR abnormalities only (based on P value)

# note this is based on transformed data (appears to increase estim prop)
